...
export const transactionController = {
    ...
    createConfig: async (req, res) => {
        ...
        try {
            ...
            const quantityRequests = items.filter((i) => i.variantId && i.quantity > 0);
            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                for (const sku of assignedSkus) {
                    ...
                    if (!instance)
                        throw new Error(`Instance ${sku} not found`);
                    // Allow RENTED if it's a re-rental check?? No, for new transaction must be AVAILABLE
                    if (instance.status !== 'AVAILABLE')
                        throw new Error(`Instance ${sku} is ${instance.status}`);
                ...
                const totalObligation = totalAmount + depositVal + parseFloat(adminFee || '0') + parseFloat(taxAmount || '0');
                const paid = payment ? parseFloat(payment.amount) : 0;
                let payStatus = 'UNPAID';
                if (paid >= totalObligation)
                    payStatus = 'PAID';
                else if (paid > 0)
                    payStatus = 'PARTIAL';
                ...
                const transactionData = {
                    ...
                    status: txStatus, // Cast to any to avoid Enum TS issues with provisional types
    ...
    addPayment: async (req, res) => {
        try {
            ...
            if (transaction.status !== 'BOOKED')
                return res.status(400).json({ error: 'Transaction Status must be BOOKED to add payment here.' });
    ...
    pickup: async (req, res) => {
        try {
            ...
            if (!transaction)
                return res.status(404).json({ error: 'Transaction not found' });
            if (transaction.status !== 'BOOKED')
                return res.status(400).json({ error: 'Transaction is not in BOOKED state' });
    ...
    returnItems: async (req, res) => {
        try {
            ...
            if (transaction.status !== 'RENTED')
                return res.status(400).json({ error: 'Transaction is not RENTED' });
            ...
            await prisma.$transaction(async (tx) => {
                ...
                if (depositAmount > 0 && totalFineAmount > 0) {
                    ...
                    // Determine Status
                    if (usedDeposit === depositAmount)
                        newDepositStatus = 'DEDUCTED'; // Fully used
                    else
                        newDepositStatus = 'PARTIAL'; // Partially used, some should be refunded
