...
export const inventoryController = {
    ...
    updateItem: async (req, res) => {
        try {
            ...
            if (brandId)
                updateData.brandId = Number(brandId);
            if (rentalPrice)
    ...
    getResume: async (req, res) => {
        try {
            ...
            const resume = variants.map(v => ({
                ...
                available: v.instances.filter((i) => i.status === 'AVAILABLE').length,
                rented: v.instances.filter((i) => i.status === 'RENTED').length,
                laundry: v.instances.filter((i) => i.status === 'IN_LAUNDRY').length,
                notReady: v.instances.filter((i) => i.status === 'NOT_READY').length,
            }));
            res.json(resume);
        }
        catch (error) {
    ...
    getHistory: async (req, res) => {
        try {
            ...
            // Combine and Sort
            const history = [...formattedStockLogs, ...formattedTransactions].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
            res.json(history);
        }
        catch (error) {
            console.error(error);
