...
async function main() {
    ...
    if (catJas && brandZara) {
        ...
        if (!item2)
            item2 = await prisma.item.create({ data: itemJas });
        ...
        if (item2 && colorHitam && sizeL) {
            ...
            if (!variant2)
                variant2 = await prisma.itemVariant.create({ data: { itemId: item2.id, sizeId: sizeL.id, colorId: colorHitam.id } });
            await prisma.itemInstance.upsert({
                where: { sku: 'JHF-L-001' },
                update: {},
        ...
        if (item2 && colorHitam && sizeXL) {
            ...
            if (!variant3)
                variant3 = await prisma.itemVariant.create({ data: { itemId: item2.id, sizeId: sizeXL.id, colorId: colorHitam.id } });
            await prisma.itemInstance.upsert({
